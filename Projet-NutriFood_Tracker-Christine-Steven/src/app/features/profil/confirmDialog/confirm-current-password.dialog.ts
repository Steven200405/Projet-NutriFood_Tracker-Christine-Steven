...
import { PasswordService } from '../../../core/storage/services/password-service';
...
@Component({
  ...
  styleUrl: './confirm-current-password.dialog.scss',
...
export class ConfirmCurrentPasswordDialog {
  ...
  async onSubmit(): Promise<void> {
    ...
    this.passwordService.passwordMatchValidator(await sha256(this.form.value.currentPassword), this.auth.getCurrentUser()?.passwordHash);
  }
}
